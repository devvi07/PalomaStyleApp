...
import { getArticulos } from '../../../utils/HttpData';
import { formatMiles } from '../../../utils/Utils';
import { useIsFocused } from '@react-navigation/native';
import FastImage from 'react-native-fast-image';

export const WomanScreen = ({ route, navigation }: any) => {
        ...
        fetch("https://pqt-calva-ws.onrender.com/api/articulos", {
            ...
            if (result.codigo == 200) {
                ...
                if (result.data && result.data.length > 0){
                    const oWoman = result.data.filter((item: any) => item.clasif == 'Mujer');
                    console.log("🚀 ~ getData ~ oWoman:", oWoman)
                    setData(oWoman);
                    setLoading(true);
                }else{
                    setData([]);
...

                                    {
                                        data.length > 0 ?
                                            ...
                                            <View style={{ flex: 1, justifyContent: 'center' }}>
                                                <Text style={{ textAlign: 'center', color: '#ff9887', fontSize: 17, fontWeight: '500' }}>No existen productos para mujer</Text>
                                            </View>
                                    }
